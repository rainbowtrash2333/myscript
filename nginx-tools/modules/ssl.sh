...
_ssl_install_to_nginx() {
    ...
    "$_ssl_acme_path" --install-cert --ecc \
        -d "$_ssl_domain" \
...
_ssl_install() {
    ...
    # 检查证书是否已申请
    if ! "$_ssl_acme_path" --list 2>/dev/null | grep -q "$_ssl_domain"; then
...
_ssl_list() {
    ...
    local _ssl_output
    _ssl_output="$("$_ssl_acme_path" --list 2>&1)" || true
